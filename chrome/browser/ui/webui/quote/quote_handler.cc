...
void QuoteHandler::OnJavascriptDisallowed() {
  weak_ptr_factory_.InvalidateWeakPtrs();
}

void QuoteHandler::RegisterMessages() {
  web_ui()->RegisterMessageCallback(
      version_ui::kRequestVersionInfo,
      base::BindRepeating(&QuoteHandler::HandleRequestVersionInfo,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      version_ui::kRequestVariationInfo,
      base::BindRepeating(&QuoteHandler::HandleRequestVariationInfo,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      version_ui::kRequestPathInfo,
      base::BindRepeating(&QuoteHandler::HandleRequestPathInfo,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      quote_ui::kRequestQuote,
      base::BindRepeating(&QuoteHandler::HandleRequestQuote,
                          base::Unretained(this)));
}

void QuoteHandler::HandleRequestVersionInfo(const base::Value::List& args) {
...
void QuoteHandler::HandleRequestVariationInfo(const base::Value::List& args) {
  AllowJavascript();
...
void QuoteHandler::HandleRequestPathInfo(const base::Value::List& args) {
  AllowJavascript();
  ...
  base::ThreadPool::PostTaskAndReply(
      ...
      base::BindOnce(&QuoteHandler::OnGotFilePaths,
                     weak_ptr_factory_.GetWeakPtr(), callback_id,
                     base::Owned(exec_path_buffer),
                     base::Owned(profile_path_buffer)));
}

void QuoteHandler::HandleRequestQuote(const base::Value::List& args) {
  AllowJavascript();
  ...
  base::Value::Dict response;
  response.Set(quote_ui::kQuote, "test quote value");
  ResolveJavascriptCallback(base::Value(callback_id), response);
}

void QuoteHandler::OnGotFilePaths(std::string callback_id,
