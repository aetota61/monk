...
#include "chrome/browser/ui/webui/quote/quote_handler.h"
...
#include "components/quote_ui/quote_ui_constants.h"
...
void CreateAndAddQuoteUIDataSource(Profile* profile) {
  WebUIDataSource* html_source =
      WebUIDataSource::CreateAndAdd(profile, chrome::kChromeUIQuoteHost);
  // These localized strings are used to label version details.
  static constexpr webui::LocalizedString kStrings[] = {
    {quote_ui::kQuote, IDS_QUOTE_UI_QUOTE},
  ...
  QuoteUI::AddVersionDetailStrings(html_source);

  html_source->UseStringsJs();
  html_source->AddResourcePath(quote_ui::kQuoteJS, IDR_QUOTE_UI_JS);
  html_source->AddResourcePath(quote_ui::kQuoteCSS,
                               IDR_QUOTE_UI_CSS);

#if BUILDFLAG(IS_ANDROID)
  html_source->AddResourcePath(quote_ui::kQuoteMobileCSS,
                               IDR_QUOTE_UI_MOBILE_CSS);
...
#endif  // BUILDFLAG(IS_ANDROID)
  html_source->SetDefaultResource(IDR_QUOTE_UI_HTML);
...
QuoteUI::QuoteUI(content::WebUI* web_ui)
...
#else
  web_ui->AddMessageHandler(std::make_unique<QuoteHandler>());
#endif
  ...
  CreateAndAddQuoteUIDataSource(profile);
}

QuoteUI::~QuoteUI() {}

// static
int QuoteUI::VersionProcessorVariation() {
#if BUILDFLAG(IS_ANDROID)
...
// static
void QuoteUI::AddVersionDetailStrings(content::WebUIDataSource* html_source) {
  ...
  html_source->AddString(
      ...
      base::i18n::MessageFormatter::FormatWithNumberedArgs(
          ...
          base::Time::Now()));
  html_source->AddString(version_ui::kCL, std::string(version_info::GetLastChange()));
...
#if !BUILDFLAG(IS_ANDROID)
// static
std::u16string QuoteUI::GetAnnotatedVersionStringForUi() {
  return l10n_util::GetStringFUTF16(
      ...
      l10n_util::GetStringUTF16(QuoteUI::VersionProcessorVariation()));
}
#endif  // !BUILDFLAG(IS_ANDROID)
