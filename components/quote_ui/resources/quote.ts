...
/**
 * Promise resolution handler for quote.
 */
// function handleQuote(
//     {quote}:
//         {quote: string}) {
//   getRequiredElement('quote').innerText = quote;
// }
...
function continueToSite(url: string) {
...
function initialize() {
  ...
  // sendWithPromise('requestQuote').then(handleQuote);
  const url_string = decodeURIComponent(window.location.search.substring(1));
  const hasAmpersand = url_string.includes('&');
  const url_quote = hasAmpersand ? url_string.substring(0, url_string.indexOf('&')) : url_string;
  const url_original = hasAmpersand ? url_string.substring(url_string.indexOf('&') + 1) : undefined;

  const quoteElement = getRequiredElement('quote');
  if (url_quote) {
    quoteElement.innerText = url_quote;
  }

  const continueElement = getRequiredElement('continue');
  if (url_original) {
    continueElement.innerText = 'Continue Anyways';
    continueElement.addEventListener('click', () => continueToSite(url_original));
  } else {
    continueElement.style.display = 'none';
  }
  const goBackElement = getRequiredElement('goback');
  if (url_original) {
    goBackElement.innerText = 'Go Back';
